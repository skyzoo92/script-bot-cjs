...
prefix = new RegExp("^(" + global.info.prefa.map(p => p.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join("|") + ")", "i")
...
const isPairing = global.staff.isPairing
...
const connectionOptions = {
	...
	version: (await (await fetch('https://raw.githubusercontent.com/WhiskeySockets/Baileys/refs/heads/master/src/Defaults/baileys-version.json')).json()).version,
...
if (usePairingCode && !conn.authState.creds.registered) {
    let phoneNumber = ''
    do {
        phoneNumber = await question("ex: 62881351692548\n\nEnter your number (must start with 62): ")
    } while (!/^\d{10,15}$/.test(phoneNumber, "raptalia") || !phoneNumber.startsWith("62"))

    try {
        const code = await conn.requestPairingCode(phoneNumber, "raptalia")
